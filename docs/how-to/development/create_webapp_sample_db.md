...
`scripts/e2e_prepare.sh` échoue avec un message explicite si un prérequis
manque (Docker arrêté, `.env` absent, port squatté, base `webapp` vide,
pont `postgres_fdw` cassé).
