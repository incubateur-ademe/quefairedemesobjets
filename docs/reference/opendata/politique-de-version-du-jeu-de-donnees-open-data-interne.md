...
> ⚠️⚠️⚠️ Créer une nouvelle version du jeu de données Open Data est un processus lourd : on évite tant que possible de créer des nouvelles versions.
>
...
les données sont diffusées sur data.ademe.fr (koumoul) et sur data.gouv.fr via un processus de mirroir (Koumoul met à jour les données sur data.gouv.fr)

## Gestion des versions mineurs : mise à jour du changelog

La mise à jour du changelog est assurée par Christian Charousset pour les évolution et modification métier (ex : ajout d'une source) et Nicolas Oudard pour les évolutions technique : évolution du schéma de données.

## Les difficultés de la mise en place d’un versionning (version majeur)
...
- Chaque version est un nouveau jeu de données sur koumoul et [data.gouv.fr](http://data.gouv.fr) → chronophage
  ...
  - sur [data.gouv.fr](http://data.gouv.fr) : c’est un nouveau fichier dans un jeu de donnée
- On doit générer et publier autant de fois qu’il y a de version
