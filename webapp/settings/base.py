...
TODO(settings-split): evolutions planned after the rename settles

  After this pure-rename branch lands, split settings into three layers
  following the Django settings package pattern:

    settings/
    ├── base.py        # shared by all environments (this file)
    ├── dev.py          # from settings.base import * + overrides
    └── production.py   # from settings.base import * + overrides

  Things to move out of base.py into the right layer:

  ▸ dev.py
    - DEBUG / STIMULUS_DEBUG / POSTHOG_DEBUG / BYPASS_ACTEUR_READONLY_FIELDS
      (developer-facing toggles)
    - EMAIL_BACKEND → console.EmailBackend
    - SHELL_PLUS_PRINT_SQL
    - MEDIA_ROOT / MEDIA_URL → local filesystem
  ...
  ▸ production.py
    - SECURE_PROXY_SSL_HEADER / USE_X_FORWARDED_HOST / USE_X_FORWARDED_PORT
      (reverse proxy headers — only meaningful behind HTTPS)
    - Sentry integration (sentry_sdk.init, DjangoIntegration)
    - S3 storage backend (AWS_* env vars → storages.backends.s3.S3Storage)
    - EMAIL_BACKEND → smtp.EmailBackend (configured via env)

  ▸ base.py cleanups
    - LOGLEVEL default: stop depending on DEBUG (use a plain default)
    - logging: only production needs the hide_staticfiles omission
"""
