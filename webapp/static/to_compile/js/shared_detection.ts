 ...
 */
export function detectIframeIntegrations(config: DetectionConfig): DetectedIframe[] {
  const {
    knownDomains,
    mainDomain,
    carteRoute,
  ...
  function detectIframeResizer(iframe: Element): boolean {
    ...
    if (
      document.documentElement.getAttribute("data-qfdmo-iframe-resizer-global") ===
      "true"
    ) {
      return true
    }
    ...
    for (const script of scripts) {
      ...
      if (
        (src.includes(carteScriptFilename) || src.includes(formulaireScriptFilename)) &&
  ...
  function processIframe(
    ...
    const hasIframeResizer = insideTemplate ? false : detectIframeResizer(iframe)
  ...
  for (const template of templates) {
    const templateIframes = (template as HTMLTemplateElement).content.querySelectorAll(
      "iframe",
    )
 ...
 */
export function detectScriptIntegrations(config: DetectionConfig): DetectedScript[] {
