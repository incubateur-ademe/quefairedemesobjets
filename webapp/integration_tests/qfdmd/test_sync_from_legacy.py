...
from qfdmd.models import HomePage, Produit, _repaired_html
...
class TestSyncFromLegacyProduit:
    def test_contenu_au_synonyme_avec_etat(self):
        ...
        assert _repaired_html(synonyme.bon_etat) in card1["description"].source
        ...
        assert _repaired_html(synonyme.mauvais_etat) in card2["description"].source
    ...
    def test_contenu_au_produit_avec_etat(self):
        ...
        assert card1["title"] == "Donner ou revendre"
        assert card2["title"] == "Déposer"
        assert _repaired_html(produit.bon_etat) in card1["description"].source
        assert _repaired_html(produit.mauvais_etat) in card2["description"].source
