...
class TestLegacyIntermediateSynonymePage:
    """Test LegacyIntermediateSynonymePage validation via clean().

    These tests simulate the modelcluster formset validation that happens
    when editing a ProduitPage in the Wagtail admin. During formset
    processing, child instances are created WITHOUT a parent page set
    (the ParentalKey is assigned after the parent is saved).
    """
    ...
    def test_add_direct_redirection_without_conflict(self, produit_page_a, db):
        ...
        intermediate = LegacyIntermediateSynonymePage(synonyme=synonyme)
        intermediate.page = produit_page_a
        intermediate.clean()
        intermediate.save()
