...
def cluster_acteurs_parent_calculations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    utilisée sur n'importe quelle df dès lors que:
     - 1 ligne = 1 acteur
     - les colonnes "id" et "parent_id" sont présentes
    """
    ...
    df["is_parent_current"] = df["id"].map(lambda x: x in parent_ids_to_count)
    df["children_count"] = df["id"].map(lambda x: parent_ids_to_count.get(x, 0))
