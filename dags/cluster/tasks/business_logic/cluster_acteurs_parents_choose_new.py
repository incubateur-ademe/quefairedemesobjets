...
def cluster_acteurs_one_cluster_parent_changes_mark(
    ...
    filter_point = (df["est_parent"] is False) & (df["identifiant_unique"] != parent_id)
    ...
    filter_delete = (df["est_parent"] is True) & (df["identifiant_unique"] != parent_id)
...
def cluster_acteurs_one_cluster_parent_choose(df: pd.DataFrame) -> tuple[str, str, str]:
    ...
    try:
        parents_count = df[df["est_parent"] is True]["identifiant_unique"].nunique()

        if parents_count == 1:
            # Case: 1 parent = we keep it
            index = df[df["est_parent"] is True].index[0]
            ...
            df_parents = df[df["est_parent"] is True]
            ...
            ids_non_parents = df[df["est_parent"] is False][
                "identifiant_unique"
            ].tolist()
