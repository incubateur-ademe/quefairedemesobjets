...
class TestEnrichLienSuccessionSuggest:
    def test_one_suggestion_per_acteur(self):
        df = pd.DataFrame(
            [
                {
    ...
    def test_champs_a_suggerer_les_deux_champs(self):
        row = pd.Series(
            {
                COLS.SIREN_ACTUEL: "123456789",
                COLS.SIRET_ACTUEL: "12345678900001",
        ...
        assert _champs_a_suggerer(row) == [
            (COLS.SIREN, "987654321"),
            (COLS.SIRET, "98765432100009"),
        ]

    def test_champs_a_suggerer_uniquement_siret(self):
        row = pd.Series(
            {
                COLS.SIREN_ACTUEL: "987654321",
                COLS.SIRET_ACTUEL: "12345678900001",
        ...
        assert _champs_a_suggerer(row) == [(COLS.SIRET, "98765432100009")]

    def test_champs_a_suggerer_uniquement_siren(self):
        row = pd.Series(
            {
                COLS.SIREN_ACTUEL: "123456789",
                COLS.SIRET_ACTUEL: "98765432100009",
        ...
        assert _champs_a_suggerer(row) == [(COLS.SIREN, "987654321")]

    def test_champs_a_suggerer_aucun_champ(self):
        row = pd.Series(
            {
                COLS.SIREN_ACTUEL: "987654321",
                COLS.SIRET_ACTUEL: "98765432100009",
