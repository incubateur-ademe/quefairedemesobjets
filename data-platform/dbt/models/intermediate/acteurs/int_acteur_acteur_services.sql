WITH norevacteur_acteur_services AS (
    ...
    INNER JOIN
        {{ ref('int_acteur') }} AS a
        ON aas.acteur_id = a.identifiant_unique AND a.revision_existe = false
...
revisionacteur_acteur_services AS (
    SELECT
        ...
        raas.acteurservice_id
    FROM {{ ref('base_revisionacteur_acteur_services') }} AS raas
    INNER JOIN
        {{ ref('int_acteur') }} AS a
        ON
            raas.revisionacteur_id = a.identifiant_unique
...
SELECT
    *,
    ROW_NUMBER() OVER (ORDER BY acteur_id, acteurservice_id) AS id
FROM acteur_acteur_services
