WITH potential_replacements AS (
    ...
    */
    ...
    INNER JOIN {{ ref('int_ae_etablissement') }} AS suggests
        ON
            candidates.etab_naf = suggests.naf
            AND candidates.etab_code_postal = suggests.code_postal
            AND candidates.etab_adresse_numero = suggests.adresse_numero
            AND suggests.adresse_normalize_string_for_match
            = {{ target.schema }}.udf_normalize_string_for_match(
                candidates.etab_adresse
            )
