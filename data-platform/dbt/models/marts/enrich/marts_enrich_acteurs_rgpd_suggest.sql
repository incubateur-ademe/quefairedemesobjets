...
unite_matching_acteurs_on_siren AS (
    SELECT
        ...
        acteurs.siren
            AS acteur_siren,
        acteurs.noms_origine
            AS acteur_noms_origine,
        acteurs.noms_normalises
            AS acteur_noms_normalises,
    ...
    LEFT JOIN acteurs_with_siren AS acteurs ON unite.siren = acteurs.siren
    WHERE
...
suggestions_with_minimum_matching_words AS (
    ...
    WHERE ( -- Any of the directors names appear in the acteur names
        ...
        OR POSITION(
            unite_dirigeant_nom_usage_normalise IN acteur_noms_normalises
        )
        > 0
        OR POSITION(
            unite_dirigeant_pseudonyme_normalise IN acteur_noms_normalises
        )
        > 0
        OR POSITION(unite_dirigeant_prenom1_normalise IN acteur_noms_normalises)
        > 0
        OR POSITION(unite_dirigeant_prenom2_normalise IN acteur_noms_normalises)
        > 0
        OR POSITION(unite_dirigeant_prenom3_normalise IN acteur_noms_normalises)
        > 0
        OR POSITION(unite_dirigeant_prenom4_normalise IN acteur_noms_normalises)
        > 0
    )
)
