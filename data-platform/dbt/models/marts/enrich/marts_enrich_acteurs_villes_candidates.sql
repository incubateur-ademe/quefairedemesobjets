...
INNER JOIN
    {{ ref('marts_ban_villes') }} AS ban
    ON acteurs.code_postal = ban.code_postal
WHERE
    acteurs.statut = 'ACTIF'
    AND acteurs.code_postal IS NOT NULL
    AND acteurs.code_postal != ''
    AND LENGTH(acteurs.code_postal) = 5
    ...
    AND (
        {{ target.schema }}.udf_normalize_string_for_match(acteurs.ville, 3)
        = {{ target.schema }}.udf_normalize_string_for_match(
            ban.ville_ancienne, 3
        )
        OR {{ target.schema }}.udf_normalize_string_for_match(acteurs.ville, 3)
        = {{ target.schema }}.udf_normalize_string_for_match(ban.ville, 3)
    )
