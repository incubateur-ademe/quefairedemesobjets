SELECT displayedacteur.*
FROM {{ ref('base_displayedacteur') }} AS displayedacteur
LEFT JOIN {{ ref('base_epci') }} AS epci ON displayedacteur.epci_id = epci.id
LEFT JOIN
    {{ ref('base_acteur_type') }} AS acteur_type
    ON displayedacteur.acteur_type_id = acteur_type.id
WHERE
    epci.code IN (
