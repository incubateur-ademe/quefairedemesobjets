...
WITH acteurs_with_siren AS (
	SELECT
		...
		warehouse.udf_normalize_string_for_match(CONCAT(nom || ' ' || nom_officiel || ' ' || nom_commercial)) AS noms_normalises,
		commentaires,
		statut
	...
	SELECT
		...
		*/
		warehouse.udf_normalize_string_for_match(dirigeant_nom) AS unite_dirigeant_nom_normalise,
		warehouse.udf_normalize_string_for_match(dirigeant_nom_usage) AS unite_dirigeant_nom_usage_normalise,
		warehouse.udf_normalize_string_for_match(dirigeant_pseudonyme) AS unite_dirigeant_pseudonyme_normalise,
		warehouse.udf_normalize_string_for_match(dirigeant_prenom1) AS unite_dirigeant_prenom1_normalise,
		warehouse.udf_normalize_string_for_match(dirigeant_prenom2) AS unite_dirigeant_prenom2_normalise,
		warehouse.udf_normalize_string_for_match(dirigeant_prenom3) AS unite_dirigeant_prenom3_normalise,
		warehouse.udf_normalize_string_for_match(dirigeant_prenom4) AS unite_dirigeant_prenom4_normalise,
		warehouse.udf_columns_concat_unique_non_empty(
			dirigeant_nom,
			dirigeant_nom_usage,
