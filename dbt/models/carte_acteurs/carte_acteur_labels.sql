WITH nochild_acteur_labels AS (
    ...
    INNER JOIN {{ ref('temp_filteredacteur') }} AS a ON al.vueacteur_id = a.identifiant_unique AND a.parent_id is null
...
parentacteur_labels AS (
    SELECT
        ...
        al.labelqualite_id AS labelqualite_id
    FROM qfdmo_vueacteur_labels al
    INNER JOIN {{ ref('temp_filteredacteur') }} AS a ON al.vueacteur_id = a.identifiant_unique AND a.parent_id is not null
