WITH nochild_acteur_acteur_services AS (
    ...
    INNER JOIN {{ ref('temp_filteredacteur') }} AS a ON aas.vueacteur_id = a.identifiant_unique AND a.parent_id is null
...
parentacteur_acteur_services AS (
    SELECT
        ...
        aas.acteurservice_id AS acteurservice_id
    FROM qfdmo_vueacteur_acteur_services aas
    INNER JOIN {{ ref('temp_filteredacteur') }} AS a ON aas.vueacteur_id = a.identifiant_unique AND a.parent_id is not null
