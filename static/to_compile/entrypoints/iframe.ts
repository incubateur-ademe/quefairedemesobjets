function compileIframeAttributes(baseUrl: string, urlParams: URLSearchParams, maxWidth: string, height: string): { [Property in keyof HTMLIFrameElement]?: unknown } {
  return ({
...
export function buildAndInsertIframeFrom(iframeAttributes: object, iframeExtraAttributes: object, scriptTag: HTMLScriptElement) {
...
export function getIframeAttributesAndExtra(initialParameters: [string, string], scriptTag: HTMLScriptElement, options?: { maxWidth: string }) {
  ...
  const iframeAttributes = compileIframeAttributes(BASE_URL, urlParams, maxWidth, height)
  return [iframeAttributes, iframeExtraAttributes]
}
const setupIframe = () => {
  // Add iFrame just after the script tag
  const scriptTag = document.currentScript as HTMLScriptElement
  const [iframeAttributes, iframeExtraAttributes] = getIframeAttributesAndExtra(["iframe", "1"], scriptTag, { maxWidth: "800px" })
