...
from dags.utils.dag_ingest_validated_utils import process_labels
...
def test_process_labels_valid_column():
    ...
    result_df = process_labels(df, "labels")
    assert_frame_equal(result_df, expected_df)


def test_process_labels_missing_column():
    data = {"other_column": [10, 20, 30, 40]}
    df = pd.DataFrame(data)
    expected_df = pd.DataFrame(columns=["acteur_id", "labelqualite_id"])

    result_df = process_labels(df, "missing_column")
    assert_frame_equal(result_df, expected_df)


def test_process_labels_empty_dataframe():
